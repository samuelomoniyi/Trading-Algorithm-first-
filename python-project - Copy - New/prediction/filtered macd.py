...
threshold = 0.1  # Start with a lower threshold, e.g., 0.1
...
# Generate Buy and Sell signals based on strong MACD crossovers
data['Buy_Signal'] = (data['MACD_Signal_Diff'] > threshold) & (data['MACD'].shift(1) <= data['Signal_Line'].shift(1))
data['Sell_Signal'] = (data['MACD_Signal_Diff'] < -threshold) & (data['MACD'].shift(1) >= data['Signal_Line'].shift(1))
...
plt.title(f'{stock_symbol} Price with Strong Buy and Sell Signals')
plt.xlabel('Date')
plt.ylabel('Price')
plt.legend()
...
plt.title('MACD with Strong Buy and Sell Signals')
plt.xlabel('Date')
plt.ylabel('MACD Value')
plt.legend()

# Plot the MACD-Signal difference to visualize the crossover strength
plt.subplot(4, 1, 4)
plt.plot(data['MACD_Signal_Diff'], label='MACD - Signal Line Difference', color='purple')
...
plt.title('MACD and Signal Line Difference')
plt.xlabel('Date')
