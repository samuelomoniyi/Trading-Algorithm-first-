...
data['Buy_Signal'] = (data['%K'] < stoch_buy_threshold) & (data['%K'].shift(1) >= stoch_buy_threshold)
data['Sell_Signal'] = (data['%K'] > stoch_sell_threshold) & (data['%K'].shift(1) <= stoch_sell_threshold)

# Check for signals
if not data['Buy_Signal'].any() and not data['Sell_Signal'].any():
    print("No buy or sell signals generated.")
else:
